...
<div class="max-w-4xl mx-auto">
    <div class="bg-white rounded-lg shadow p-6">
        <form action="{{ route('recuento.store') }}" method="POST" id="sobreForm">
            ...
            @if($culto->cerrado)
            <div class="mb-6 p-4 bg-amber-50 border border-amber-300 rounded-lg flex items-start gap-3">
                <svg class="w-5 h-5 text-amber-600 mt-0.5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                    <path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd"></path>
                </svg>
                <div class="text-sm text-amber-800">
                    <p class="font-semibold">Este culto ya está cerrado.</p>
                    <p>El sobre se agregará como corrección: los totales se recalcularán y el movimiento
                       quedará registrado en la bitácora. Los PDF del recuento ya firmados no cambian solos,
                       así que conviene volver a descargarlos si se necesitan actualizados.</p>
                </div>
            </div>
            @endif
            @endif
            ...
            <div class="mt-6 flex justify-end gap-3 sticky bottom-0 bg-white pb-[env(safe-area-inset-bottom,16px)] pt-4 border-t border-gray-200 -mx-6 px-6 z-10" style="padding-bottom: max(1rem, env(safe-area-inset-bottom));">
